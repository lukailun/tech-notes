...
> 注：值类型的包装体"存在变量所在之处"——在局部作用域默认在栈；若作为类的属性，则内联在该类的堆内存中；被逃逸闭包捕获时也可能移到堆。下文各节"在栈"均指此默认情况。
...
值类型包装体（16 字节）默认在栈（局部作用域）；若作为类的属性则内联在该类堆内存中，被逃逸闭包捕获也可能移到堆。其内容位置由长度决定：短字符串（UTF-8 字节数小于等于 15）内容直接存在栈，长字符串（UTF-8 字节数大于 15）内容存在堆。可以使用 `utf8.count` 查看字符串实际内容占用的字节数。
...
值类型包装体（8 字节）默认在栈（局部作用域），真实数据存在堆，栈上仅存储指向堆内存的指针；若作为类的属性则随类内联在堆中。
