...
* 值类型：结构体（Struct）、枚举（Enum）、函数（Function）、非逃逸闭包（Non-escaping Closure）。
* 引用类型：类（Class）、参与者（Actor）、逃逸闭包（Escaping Closure）。

## 栈 (Stack) & 堆 (Heap)

Swift 中两种核心内存分配方式，由编译器自动根据数据类型决定使用。

* 栈：运行时可预测分配，采用后进先出（LIFO）机制。
* 堆：运行时动态分配，需通过自动引用计数（ARC）管理内存，机制复杂。

> 注：值本身存在变量所在之处——在局部作用域默认在栈；若作为类的属性，则内联在该类的堆内存中；被逃逸闭包捕获时也可能移到堆。下文各节“在栈”均指此默认情况。
...
String 值本身（16 字节）默认在栈（局部作用域）；若作为类的属性则内联在该类堆内存中，被逃逸闭包捕获也可能移到堆。其底层存储（内容）位置由长度决定：短字符串（UTF-8 字节数小于等于 15）内容直接存在栈，长字符串（UTF-8 字节数大于 15）内容存在堆。可以使用 `utf8.count` 查看字符串实际内容占用的字节数。

> 注意：15 是 UTF-8 字节上限，不是字符数。含多字节 UTF-8 字符（如中文）会更快占满该上限。
...
集合值本身（8 字节）默认在栈（局部作用域），底层存储（元素数据）在堆，值本身仅持有指向它的指针；若作为类的属性则随类内联在堆中。

## `Struct`

结构体是值类型，内容默认在栈（局部作用域）；若作为类的属性或被逃逸闭包捕获，可能移到堆。内存大小由成员总和与对齐规则决定。
...
类是引用类型，内容存在堆。引用（8 字节堆指针）默认在栈（局部作用域）存储，与成员数量无关。可以使用 `class_getInstanceSize` 查看类实例在堆中实际占用的字节数。
...
枚举是值类型，内容默认在栈（局部作用域）。无关联值的枚举固定占用 1 字节，有关联值时大小由最大关联值决定。
...
`Optional` 本质是枚举，属于值类型，默认在栈（局部作用域）存储。
...
参与者是引用类型，内容存在堆。引用（8 字节堆指针）默认在栈（局部作用域）存储，与成员数量无关。可以使用 `class_getInstanceSize` 查看参与者实例在堆中实际占用的字节数。`Actor` 底层为了实现并发安全与数据隔离，内置专门的运行时机制，因此堆内存占用远大于普通 `Class`。
