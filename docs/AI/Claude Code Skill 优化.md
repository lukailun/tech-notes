# Claude Code Skill 优化

最近使用 Claude Code 为自身工作流开发多个 Skill 时，实践中发现一个问题：Skill 写得越详细，响应质量反而越差。排查后发现，根本原因是自己一直用编写文档的思路编写 Skill。
...
Claude Code 针对 Skill 的加载采用分层加载、按需调用的渐进式披露机制，核心目标是节省上下文 token、提升响应效率。

- metadata 常驻上下文：对话启动时，Claude Code 会将所有 Skill 的 metadata 加载并常驻在上下文中，用于快速识别 Skill。
- SKILL.md 按需加载：当用户的请求内容命中某个 Skill 时，Claude Code 会将该 Skill 对应的 SKILL.md 正文加载到上下文，以加载当前任务所需的核心指令。
...
针对低自由度及关键任务，使用脚本执行可解决 Agent 直接执行的痛点，核心优势有三点：
...
Claude Code 的[技能编写最佳实践](https://platform.claude.com/docs/en/agents-and-tools/agent-skills/best-practices)核心思想可以总结成一句话：

> 默认假设：Claude 已经非常聪明。只添加 Claude 不知道的上下文。(Default assumption: Claude is already very smart. Only add context Claude doesn't already have.)
...
* 执行步骤的详细说明：对脚本功能逐条列举，Agent 执行时无需知晓脚本内部功能细节，属于冗余描述。
* 重复的要求说明：在“要求”中已提及 glab CLI 安装配置和环境变量配置，却在“执行步骤”中再次重申，属于无效冗余。
* 典型的防御性写作：“注意事项”本质上是提供给人类阅读的提醒，Agent 执行脚本时无需额外知晓这些提醒，无需常驻上下文。
* 非必要的背景描述：“自动创建 GitLab 合并请求,包含智能目标分支选择和 AI 生成的描述。”属于背景性内容，不属于 Agent 执行的核心步骤，可移至参考文档，无需常驻上下文。
